...
test.describe('Choosing a team', { tag: '@spec-0017' }, () => {
  test('a deep link sends a teamless player to the choice screen and back', async ({
    ...
    // and the reason (rule 12) — never silently absent.
    ...
    // …and the guest lands where they were going all along (rule 9).
  ...
  test('a captain is told they are one, and is offered nothing', async ({ browser }) => {
    // Rule 7: a captain is seeded onto their own team and cannot leave it.
...
test.describe('What a match moves', { tag: '@spec-0017' }, () => {
  test('a match between the two teams moves both leaderboards', async ({ browser }) => {
    ...
    // The two leaderboards are never added together (rule 25).
  ...
  test('a match inside one team moves only the player leaderboard', async ({
    ...
    // Rule 19: a team cannot farm itself with its own internal pairs.
